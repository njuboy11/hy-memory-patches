...
def _lemmatize_english(text: str) -> str:
    ...
    for token in doc:
        ...
        lemma = token.lemma_
        # Allow underscores (phrase-dict produces e.g. dense_embedding)
        if lemma.replace('_', '').isalnum() or '_' in lemma:
            tokens.append(lemma)
...
def _load_phrase_dict() -> None:
    """Lazy-load multi-word phrase dictionary for English.

    Format: one phrase per line, words separated by spaces.
    Example: 'dense embedding' → preprocessed to 'dense_embedding'
    """
    global _phrase_dict, _phrase_dict_loaded
    if _phrase_dict_loaded:
        return
    _phrase_dict_loaded = True

    import os
    path = os.environ.get("MEMORY_SPACY_PHRASEDICT", "")
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not line or line.startswith("#"):
                    continue
                phrase = " ".join(line.split())  # normalize whitespace
                joined = "_".join(line.split())
                if phrase and joined and phrase != joined:
                    _phrase_dict[phrase] = joined
        if _phrase_dict:
            logger.info(
                f"[lemmatize] phrase dict loaded: {path} ({len(_phrase_dict)} entries)"
            )
    except Exception as e:
        logger.warning(f"[lemmatize] phrase dict load failed: {e}")
        _phrase_dict = {}


def _preprocess_phrases(text: str) -> str:
    """Replace known multi-word phrases with underscore-joined versions.

    Runs before lemmatization so spaCy treats the result as a single token.
    """
    _load_phrase_dict()
    if not _phrase_dict or not text:
        return text
    # longest-first to avoid partial matches (e.g. 'sparse vector' before 'vector')
    for phrase in sorted(_phrase_dict, key=len, reverse=True):
        text = text.replace(phrase, _phrase_dict[phrase])
    return text
...
def lemmatize_for_bm25(text: str) -> str:
    ...
    # Preprocess: replace multi-word phrases (e.g. 'dense embedding' → 'dense_embedding')
    text = _preprocess_phrases(text)
